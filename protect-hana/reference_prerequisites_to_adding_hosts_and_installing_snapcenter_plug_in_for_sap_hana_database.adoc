...
summary: Antes de agregar un host e instalar los paquetes de complementos, debe completar todos los requisitos.  El complemento de SnapCenter para la base de datos SAP HANA está disponible en entornos Windows y Linux. 
---
= Requisitos previos para agregar hosts e instalar el complemento de SnapCenter para la base de datos SAP HANA
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Antes de agregar un host e instalar los paquetes de complementos, debe completar todos los requisitos.  El complemento de SnapCenter para la base de datos SAP HANA está disponible en entornos Windows y Linux.
...
* Debe tener instalado el terminal interactivo de base de datos SAP HANA (cliente HDBSQL) en el host.
* Para Windows, el servicio de creación de complementos debe ejecutarse utilizando el usuario de Windows "`LocalSystem`", que es el comportamiento predeterminado cuando el complemento para la base de datos SAP HANA se instala como administrador de dominio.
* Para Windows, las claves del almacén de usuarios deben crearse como usuario del SISTEMA.
* Al instalar un complemento en un host de Windows, si especifica una credencial que no está incorporada o si el usuario pertenece a un usuario de grupo de trabajo local, debe deshabilitar UAC en el host.  El complemento de SnapCenter para Microsoft Windows se implementará de forma predeterminada con el complemento SAP HANA en los hosts de Windows.
* En el caso del host Linux, se accede a las claves de HDB Secure User Store como usuario del sistema operativo HDBSQL.
* El servidor SnapCenter debe tener acceso al puerto 8145 o personalizado del complemento para el host de base de datos SAP HANA.
...
* Al instalar el complemento para la base de datos SAP HANA en un host Windows, el complemento de SnapCenter para Microsoft Windows se instala automáticamente.
...
* Para las bases de datos SAP HANA que se ejecutan en un host Linux, al instalar el complemento para la base de datos SAP HANA, el complemento de SnapCenter para UNIX se instala automáticamente.
...
Para ejecutar un comando complementario en el complemento de SnapCenter para SAP HANA, debe incluirlo en el archivo _allowed_commands.config_.
...
Comando: "C:\Archivos de programa\ NetApp\SnapCreator commands\sdcli.exe"
